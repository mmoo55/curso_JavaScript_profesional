...
class Ads {
  ...
  private initAds() {
    this.ads = [...ALL_ADS];    /* Creamos una copia intacta utilizando destructuración, tomamos todos los elementos del arreglo y lo dejamos caer sobre otra caja (nuevo arreglo) */
  }

  getAd() {
    if (this.ads.length === 0) {    /* Verificamos si los ads se acabaron de la lista */
      this.initAds();               /* Si se acabaron de la lista lo volvemos a inicializar */
    }
